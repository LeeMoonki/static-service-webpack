...
export const select_props = curry((props = {}, v) => (isUndefined(props[v]) ? '' : props[v]));

export const scat_props = props => scat(([k, v]) => `${k}="${v}" `, entries(props));

export const styles = props => scat(([k, v]) => `${k}: ${v}; `, entries(props));

export const array_has_value = curry((v, arr) => go(arr, some(_ => _ == v)));

// ## 인동 flatten 생겼어요.
export const flatten_array = (arr, _arr = []) => {
  (function recur(arr) {
    for (const each of arr) {
      if (isArray(each)) recur(each);
      else _arr.push(each);
    }
  })(arr);
  return _arr;
...
export const stopLog = d => new Promise(() => log(d));
